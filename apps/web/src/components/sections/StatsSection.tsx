        ...
        <ScrollReveal className="space-y-8 lg:space-y-10">
          {(section.eyebrow || section.title || section.intro) ? (
            <div className="max-w-3xl space-y-4">
              ...
              {section.intro ? <p className={cn('max-w-[60ch] text-[clamp(1rem,1.8vw,1.125rem)] leading-[1.8] font-light', getSectionMutedTextClass(section.theme))}>{section.intro}</p> : null}
            </div>
          ) : null}
          <div className={cn('grid grid-cols-1 gap-4 md:gap-5 lg:gap-6', gridClass)}>
            {section.stats.map((stat) => (
              <div key={stat._key ?? `${stat.label}-${stat.value}`} className={cn('rounded-[1.75rem] border p-6 md:p-7 lg:p-8', getSectionCardClass(section.theme))}>
                <div className="font-display text-[clamp(2.5rem,5vw,4.5rem)] leading-none text-accent">
                  <CounterAnimation value={stat.value} />
                </div>
                <div className={cn('mt-3 text-sm font-bold uppercase tracking-[0.2em]', getSectionSubtleTextClass(section.theme))}>{stat.label}</div>
              </div>
            ))}
          </div>
        </ScrollReveal>
